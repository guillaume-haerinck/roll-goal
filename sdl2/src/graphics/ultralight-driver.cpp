...
#include "shaders/ultralight/shader_fill_frag.h"
#include "shaders/ultralight/shader_fill_path_frag.h"
#include "shaders/ultralight/shader_v2f_c4f_t2f_t2f_d28f_vert.h"
#include "shaders/ultralight/shader_v2f_c4f_t2f_vert.h"

#define SHADER_PATH "res/shaders/ultralight"
...
UltralightDriver::UltralightDriver(GLfloat scale) : scale_(scale)
{
  // Render Buffer ID 0 is reserved for the screen
  frame_buffer_map[0] = 0;
}

void UltralightDriver::CreateTexture(uint32_t texture_id,
                                Ref<Bitmap> bitmap)
{
  GLuint tex_id;
...
void UltralightDriver::UpdateTexture(uint32_t texture_id,
                                Ref<Bitmap> bitmap)
{
  glActiveTexture(GL_TEXTURE0 + 0);
...
void UltralightDriver::BindTexture(uint8_t texture_unit, uint32_t texture_id)
{
...
void UltralightDriver::DestroyTexture(uint32_t texture_id)
{
  GLuint tex_id = texture_map[texture_id];
  glDeleteTextures(1, &tex_id);
}

void UltralightDriver::CreateRenderBuffer(uint32_t render_buffer_id,
...
void UltralightDriver::BindRenderBuffer(uint32_t render_buffer_id)
{
  glBindFramebuffer(GL_FRAMEBUFFER, frame_buffer_map[render_buffer_id]);
  CHECK_GL();
}

void UltralightDriver::ClearRenderBuffer(uint32_t render_buffer_id)
{
...
void UltralightDriver::DestroyRenderBuffer(uint32_t render_buffer_id)
{
  if (render_buffer_id == 0)
    return;
...
void UltralightDriver::CreateGeometry(uint32_t geometry_id,
...
void UltralightDriver::UpdateGeometry(uint32_t geometry_id,
                                 const VertexBuffer &vertices,
                                 const IndexBuffer &indices)
{
...
void UltralightDriver::DrawGeometry(uint32_t geometry_id,
...
void UltralightDriver::DestroyGeometry(uint32_t geometry_id)
{
  GeometryEntry &geometry = geometry_map[geometry_id];
...
void UltralightDriver::UpdateCommandList(const CommandList &list)
{
  if (list.size)
  {
...
void UltralightDriver::DrawCommandList()
{
...
void UltralightDriver::BindUltralightTexture(uint32_t ultralight_texture_id)
{
  GLuint tex_id = texture_map[ultralight_texture_id];
  glBindTexture(GL_TEXTURE_2D, tex_id);
}

void UltralightDriver::LoadPrograms(void)
{
  LoadProgram(ultralight::kShaderType_Fill);
  LoadProgram(ultralight::kShaderType_FillPath);
}

void UltralightDriver::DestroyPrograms(void)
{
...
void UltralightDriver::LoadProgram(ProgramType type)
{
  GLenum ErrorCheckValue = glGetError();
  ProgramEntry prog;
...
void UltralightDriver::SelectProgram(ProgramType type)
{
...
void UltralightDriver::UpdateUniforms(const GPUState &state)
{
  float params[4] = {(float)(SDL_GetTicks() / 1000.0), state.viewport_width, state.viewport_height, scale_};
...
void UltralightDriver::SetUniform1fv(const char *name, size_t count, const float *val)
{
  glUniform1fv(glGetUniformLocation(cur_program_id_, name), (GLsizei)count, val);
}

void UltralightDriver::SetUniform4f(const char *name, const float val[4])
{
  glUniform4f(glGetUniformLocation(cur_program_id_, name),
              (GLfloat)val[0], (GLfloat)val[1], (GLfloat)val[2], (GLfloat)val[3]);
}

void UltralightDriver::SetUniform4fv(const char *name, size_t count, const float *val)
{
  glUniform4fv(glGetUniformLocation(cur_program_id_, name), (GLsizei)count, val);
}

void UltralightDriver::SetUniformMatrix4fv(const char *name, size_t count, const float *val)
{
  glUniformMatrix4fv(glGetUniformLocation(cur_program_id_, name), (GLsizei)count, false, val);
}

void UltralightDriver::SetViewport(float width, float height)
{
